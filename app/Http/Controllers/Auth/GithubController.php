    ...
    public function handleGitHubCallback()
    {
        try {
            ...
            if (!$user) {
                ...
                $user->password = Hash::make($githubUser->getEmail());
                $user->provider = 'github';
                $user->provider_id = $githubUser->getId();
                $user->email_verified_at = now();
                $username = $this->generateUsername($githubUser->getName() ?? $githubUser->getNickname());
                ...
                app('notificationService')->notifyUsers([$user], 'You have been registered from github and your password is your email please change it to secured one from your profile security settings.');
            }
            ...
            $profile = Profile::updateOrCreate(
                ['user_id' => $user->id],
                [
                    'first_name'    => $githubUser->getName() ?? $githubUser->getNickname(),
            ...
            if ($githubUser->getAvatar()) {
                ...
                $profileImageUrl = $githubUser->getAvatar(); // Get github avatar URL
            ...
            DB::commit();
            return redirect()->route('dashboard')->with('success', 'GitHub login successfully.');
        } catch (\Exception $e) {
            DB::rollBack();
            Log::error('GitHub login error:', ['error' => $e->getMessage()]);
